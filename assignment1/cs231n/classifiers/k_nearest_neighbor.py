...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        training point in self.X_train using a nested loop over both the
        ...
        dists[i, j] is the Euclidean distance between the ith test point
    ...
    def compute_distances_no_loops(self, X):
        ...
        num_train = self.X_train.shape[0]
        dists = np.zeros((num_test, num_train))
        # ||x-y||^2 = <x-y>T<x-y> = xTx + yTy - 2 xTy
        XX = np.sum(X ** 2, axis=1, keepdims=True)
        YY = np.sum(self.X_train ** 2, axis=1, keepdims=True).T
        dists = XX + YY - 2 * X.dot(self.X_train.T)
        return dists
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in xrange(num_test):
            ...
            closest_y_bins = np.bincount(closest_y)
            y_pred[i] = np.argmax(closest_y_bins)
        return y_pred
